...
OUTPUT_FILE = "trilingual_test_results.md"

# Import queries list from test_50_trilingual_queries.py
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
try:
    from test_50_trilingual_queries import TRILINGUAL_QUERIES
except ImportError as e:
    print("Error: Could not import TRILINGUAL_QUERIES from test_50_trilingual_queries.py:", e)
    sys.exit(1)
...
CATEGORIES = [
    ...
    {"name": "5. Ordinary Gazette Notification for Name Change", "range": range(32, 42)},
    {"name": "6. Out of Scope Queries", "range": range(42, 50)}
]
...
def main():
    print(f"=== Starting Trilingual Evaluation Runner ===")
    print(f"Total query triplets (EN, HI, Hinglish): {len(TRILINGUAL_QUERIES)}")
    print(f"Output markdown file will be written to: {OUTPUT_FILE}")
    print("-" * 60)
    
    # Initialize output file
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write("# SewaSetu RAG Chatbot Trilingual Evaluation Report\n\n")
        f.write("This report presents the testing results of the chatbot in three languages: **English**, **Hindi**, and **Hinglish** across all 5 in-scope citizen services and out-of-scope scenarios. Queries were executed globally without specifying service serial numbers.\n\n")
        f.write("## Test Configuration\n")
        f.write(f"- **API Server:** `{API_URL}`\n")
        f.write("- **Date & Time:** " + time.strftime("%Y-%m-%d %H:%M:%S") + "\n")
        f.write(f"- **Total Query Runs:** {len(TRILINGUAL_QUERIES) * 3} (50 English + 50 Hindi + 50 Hinglish)\n\n")
    ...
    for cat in CATEGORIES:
        ...
        for idx in cat["range"]:
            if idx >= len(TRILINGUAL_QUERIES):
                continue
            item = TRILINGUAL_QUERIES[idx]
            q_num = idx + 1
    ...
    print("\n=== Trilingual Evaluation Suite Completed Successfully! ===")
