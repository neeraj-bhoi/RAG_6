...
# Point to "data" folder which holds the scraped data in our current workspace
...
def run_ingestion():
    ...
    # 1. Dynamically discover service IDs from scraped profiles
    profiles_dir = os.path.join(SCRAPED_DATA_DIR, "profiles")
    service_ids = []
    if os.path.exists(profiles_dir):
        for filename in os.listdir(profiles_dir):
            if filename.startswith("service_") and filename.endswith("_en.json"):
                ...
                if len(parts) >= 3:
                    try:
                        ...
                        pass
    service_ids.sort()
    print(f"[Processor] Discovered service IDs: {service_ids}")
    
    # 2. Compile manifest catalog from active profiles
    ...
    for idx, sid in enumerate(service_ids):
        ...
        en_profile_path = os.path.join(profiles_dir, f"service_{sid}_en.json")
        hi_profile_path = os.path.join(profiles_dir, f"service_{sid}_hi.json")
        ...
        if os.path.exists(en_profile_path):
            try:
                ...
                print(f"[Processor] Error loading English profile for service {sid}: {e}")
        ...
        if os.path.exists(hi_profile_path):
            try:
                ...
                print(f"[Processor] Error loading Hindi profile for service {sid}: {e}")
                
        # Locate manuals if they exist in data/extracted_text
        manual_rel_en = None
        manual_rel_hi = None
        ...
        if os.path.exists(os.path.join(SCRAPED_DATA_DIR, "extracted_text", manual_filename_en)):
            manual_rel_en = f"data/extracted_text/{manual_filename_en}"
        if os.path.exists(os.path.join(SCRAPED_DATA_DIR, "extracted_text", manual_filename_hi)):
            manual_rel_hi = f"data/extracted_text/{manual_filename_hi}"
        ...
        services_manifest.append({
            ...
            "path_en": f"data/profiles/service_{sid}_en.json",
            "path_hi": f"data/profiles/service_{sid}_hi.json",
    ...
    # 3. Ingest structured JSON profiles (converted to Markdown)
    for s in services_manifest:
        ...
        for lang in LANGUAGES:
            ...
            try:
                ...
                # Format JSON to clean markdown, excluding form fields to optimize prompt size
    ...
    if total_docs == 0:
        print("[Processor] Warning: No documents found to ingest!")
        return
